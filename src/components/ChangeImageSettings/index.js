...
function ChangeImageSettings(props) {
  ...
  return (
    <div className="ChangeImageSettings">
      <EditorButton
        clickAction={() => {
          setModalOpen(!modalOpen)
          setPrevWidth(selectedItem.width)
          setPrevHeight(selectedItem.height)
        }}
          ...
          contents={
            <div className="modalContents">
              <SettingInput
                ...
                changeAction={
                  (e) => {
                    selectedItem.resizeXScale = prevWidth / Number(e.target.value)
                    selectedItem.width = Number(e.target.value)
                    dispatch(updateObject(selectedItem))

                    if (scaleProportionally) {
                      selectedItem.resizeYScale = prevHeight / Math.round(Number(e.target.value * selectedItem.sizeProportion))
